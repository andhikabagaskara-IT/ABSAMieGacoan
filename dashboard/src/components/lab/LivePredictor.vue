...
const predict = () => {
  ...
  setTimeout(() => {
    ...
    // Simple naive logic for simulation
    if (text.includes('kecewa') || text.includes('lama') || text.includes('asin') || text.includes('kotor') || text.includes('mahal') || text.includes('buruk')) {
      sentiment = 'Negatif'
    }
    
    if (text.includes('mie') || text.includes('rasa') || text.includes('enak') || text.includes('asin')) {
      aspects.push('Rasa & Bumbu')
    }
    if (text.includes('tempat') || text.includes('bersih') || text.includes('kotor') || text.includes('nyaman')) {
      aspects.push('Tempat & Suasana')
    }
    if (text.includes('layan') || text.includes('nunggu') || text.includes('lama') || text.includes('ramah')) {
      aspects.push('Pelayanan & Waktu Tunggu')
    }
    
    if (aspects.length === 0) {
      aspects.push('Kualitas Umum')
    }
    
    result.value = {
      sentiment,
      confidence: (Math.random() * 15 + 84).toFixed(1), // Random 84-99%
      aspects
    }
