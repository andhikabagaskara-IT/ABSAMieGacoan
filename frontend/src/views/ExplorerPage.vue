  ...
  <div class="explorer-page">
    ...
    <div class="card table-container">
      <div class="table-header-bar">
        ...
        <button v-if="canExport" class="btn btn-primary export-btn" @click="exportCsv">
...
const { branchList, getAspectLabel, ASPECT_LABELS, totalReviews } = useDashboardData()
const authStore = useAuthStore()
const canExport = computed(() => ['admin', 'analyst'].includes(authStore.user?.role?.toLowerCase()))
...
const totalAll = computed(() => totalReviews.value || 0)
const paginatedData = computed(() => dataRows.value)
...
const fetchReviews = async () => {
  ...
  try {
    const params = {
      ...
      aspect: filterAspect.value,
      rating: filterRating.value,
      sort_by: sortKey.value,
      sort_dir: sortDir.value
    }
    const res = await api.get('/reviews', { params })
    dataRows.value = res.data.reviews
    totalFiltered.value = res.data.pagination.total
...
// Custom debounce for search input
let searchTimeout
const onSearchInput = () => {
...
const resetFilters = () => {
  ...
  filterAspect.value = ''; filterRating.value = ''; sortKey.value = 'id'; sortDir.value = 'desc'
  onFilterChange()
}
...
const exportCsv = async () => {
  try {
    ...
    if (filterBranch.value) params.append('branch', filterBranch.value)
    if (filterSentiment.value) params.append('sentiment', filterSentiment.value)
    if (filterAspect.value) params.append('aspect', filterAspect.value)
    
    // Melakukan GET dari Axios tapi responsenya berupa blob
    const response = await api.get(`/pipeline/export-csv?${params.toString()}`, {
      responseType: 'blob'
    })
    ...
    link.href = url
    link.setAttribute('download', 'ulasan_mie_gacoan_export.csv')
